...
| Database | SQLite (dev) / PostgreSQL (optional, for multi-user/prod) |
| ORM | SQLAlchemy |
...
```
     ...
     Market Data     ML Engine        Database
      (yfinance)   (Scikit-learn)   (SQLite/Postgres)
          │              │
          ▼              ▼
...
Frontend runs at `http://localhost:5173` (or `http://localhost:3000` depending on your setup)
...
| GET | `/api/performance/{symbol}` | Model accuracy/precision/recall/F1 + backtest results |
...
3. **Target**: `1` if next day's close > today's close, else `0` (direction classification); optional regression target for exact price
4. **Validation**: `TimeSeriesSplit` (chronological, no shuffling) — critical to avoid lookahead bias
5. **Models compared**: Logistic Regression (baseline), Random Forest, XGBoost (optional)
6. **Metrics**: Accuracy, Precision, Recall, F1, ROC-AUC (classification); MAE, RMSE, MAPE, R² (regression)
...
**Random Forest CV Metrics (TimeSeriesSplit)**
- Accuracy: 49.4%
- Precision: 0.53
- Recall: 0.38
- F1 Score: 0.39

**Backtest (1-year test period, ₹100,000 starting capital)**
- Buy & Hold: ₹126,305.78 (26.3%)
- Model Strategy (UP-only): ₹100,148.86 (0.1%)
